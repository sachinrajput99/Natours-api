      ...
      while (1) switch (_context.prev = _context.next) {
        ...
        case 11:
          ...
          if (data.status === 'success') {
            (0, _alerts.showAlert)('success', 'Logged in Successsssfully');
...
var displayMap = exports.displayMap = function displayMap(location1) {
  //map initialization
  var map = L.map('map').setView([34.0522, -118.2437], 10); // [latitude, longitude] (LA coordinates)
  ...
  location1.forEach(function (location) {
    ...
    var marker = L.marker([location.coordinates[1], location.coordinates[0]]).addTo(map);

    // Extend bounds with current location
    bounds.extend([location.coordinates[1], location.coordinates[0]]);

    // Create a popup
    marker.bindPopup("<p>".concat(location.day, ": ").concat(location.description, "</p>"));
  });
  console.log(bounds);
...
//polyfills this code so as to run in older browser
...
//DELEGATION
if (mapBox) {
  var location1 = JSON.parse(document.getElementById('map').dataset.locations);
